...
## B09–B12 integration decisions

- The latest instruction prohibits all Git actions. B09–B12 are implemented together in the existing checkout; no Git command, branch change, commit, push or PR action is part of this completion gate.
- B09 correct code is retained. Its server-priced demo adapter, transactional activation, snapshots, retry idempotency, calendar periods and annual allocation conservation are regression-tested. Billing and scores require a replica set; environment examples now make that topology explicit.
- Score dates are ISO date-only strings, not instants. UTC defines today for validation, and display uses string formatting without timezone conversion. Values are integers 1–45. A unique compound index enforces one score per user/date. All mutations acquire a per-user transactional write through `scoreRevision`; insertion and oldest deletion commit together. No committed six-row state is visible. Full-set dates older than the oldest retained round are rejected for both creates and edits. Admin work must reuse the validator and transactional service, not bypass them.
- Profile permits only name and date-display preference. Email/password changes need a separate verified flow; role, suspension, ownership and token fields cannot be patched. Charity preferences keep their existing separately validated endpoint.
- Dashboard current membership, newest scores and selected charity are separate source reads with individual error states. Draw date, participation and winnings are unavailable/null, not invented dates or zero totals. Five scores indicate only the score requirement, never an automatic draw entry.
- Existing forest/cream/gold design tokens and supplied score/dashboard/profile visual references are reused. Demo-only static totals and export scripts are not copied into app behavior. Home entrance motion now changes position only, keeping text contrast stable during accessibility checks. Reduced-motion behavior remains supported.

## B13–B26 local integration decisions (supersedes older availability notes)

- The successive requests extend the same workspace work. No Git commands/actions, production connections, remote CI, release/tag or deployment were performed. Older branch/commit references above are historical only.
- Preserve existing JavaScript/Express/Mongoose/React architecture and forest/cream/gold design tokens. Monthly draw and winnings reference screens inform presentation, but illustrative prizes, fake proof success and export scripts are not copied as live behavior. Admin tables favor readable accessible data and audited controls over invented dashboard metrics.
- Draw engine uses secure `crypto.randomInt`, five numbers without replacement and highest distinct-match tier only. Weighted sampling uses baseline weight one plus distinct frequency across eligible entries; this keeps every number possible. Tests inject deterministic integer choices only into services, never through a browser route.
- Pool split is integer 25/35/40; floor three/four tiers and give split residual to five tier. Divide winners equally with remainder pennies ordered by user ID. Only unclaimed five tier rolls; unclaimed three/four amounts remain separately accounted. These explicit rules settle rounding/duplicate ambiguities without inventing a payout provider.
- Global transactional eligibility locking serializes score/subscription/admin changes with publication. Preview fingerprints include current revisions and cutoff snapshots, successful allocation ledger and configuration. Publish persists the reviewed numbers, entry snapshots, winners/payouts and one currency rollover in one transaction. Fixed cutoff history begins when configured; backdated reconstruction is rejected. The bounded demo capacity is 1000 accounts. One draw per month and chronological publication prevent consuming a jackpot twice.
- Every winning proof is private. Local adapter stores actual sanitized image bytes in MongoDB and is labelled accurately. Cloudinary integration uses authenticated uploads, verified signatures and server-proxied short-lived reads. A durable staged/cleanup record handles external deletion failures. Real Cloudinary account/credentials were not used; external validation remains outstanding.
- Verification and payout are independent states. Rejection/resubmission preserves every submission and reason; approval gates exactly one recorded manual/demo payout with unique reference, actor and time. No frontend action can claim a bank transfer.
- Admin profile, score and entitlement writes are allowlisted and audited. Self-suspension/final usable admin removal is guarded transactionally. Manual-demo subscription adjustments create entitlement history, never provider payment/ledger entries. Referenced charities are archived, not deleted. Active-selection reservation sets a conservative private reference flag to close selection/deletion races; an interrupted attempt may retain archival protection safely.
- Reports separate annual monthly allocations, independent donations, awards, payments and current rollover. UTC inclusive date inputs become exclusive next-midnight bounds. Monetary currencies never merge. Current account/entitlement/rollover balances are explicitly distinguished from date-filtered activity.
- Session-loss events clear private route state and invalidate in-flight auth refresh generations. Error recovery never automatically retries payment/draw writes. Known operational errors are sanitized; schema/duplicate errors have consistent codes. Typed input survives failed requests; focus moves to actionable error guidance.
- B25/B26 are configuration/local verification only. Single-origin proxy arrangement is preferred over cross-site cookie workarounds. Production requires HTTPS, bounded proxy trust, explicit additive indexes, disabled simulated payments and non-local media. The unresolved Vercel `.invalid` API destination makes the configuration visibly unfinished for hosting.
- PRD conflicts remain explicit: MongoDB/Mongoose is implemented rather than Supabase; real billing/webhooks/banking and secure email verification/recovery/new-account services are absent. Production admin provisioning and external monitoring/backups are not created. No production-readiness claim is made.
